...
public class CIStretchCropFilter {
    ...
    public init(size: CIVector = CIVector(values: [1280.0, 720.0], count: 2),
                image: CIImage,
...
extension CIImage {
    ...
    public func stretchCropFilter(size: CIVector = CIVector(values: [1280.0, 720.0], count: 2),
                                  cropAmount: NSNumber = 0.25,
                                  centerStretchAmount: NSNumber = 0.25) -> CIImage? {
        guard let filter = CIFilter(name: "CIStretchCrop") else { return nil }
